...
int main() {
    ...
    COLUMN **cdf;
    int nb_col;
    if (cdataframe_dur==1){ //Si l'utilisateur a choisi d'utiliser le CDataFrame en dur, on définit le cdf de toutes les fonctions sur celle-ci
        cdf=cdf_dur;
        nb_col=cdf_dur_nb_col;
    }
    else{   // Remplissage du CDataframe à partir des saisies utilisateurs
        do {
            printf("Entrez le nombre de colonne souhaite :");
            scanf("%d", &nb_col);
        } while ((nb_col) < 0); //Saisie sécurisée taille CDataFrame
        ...
        for(int i=0; i<nb_col; i++) {   //Création d'une colonne et implémentation dans le CDataFrame
            ...
            printf("Entrez le titre de la colonne %d :", i);  //Saisie titre de la colonne
            scanf("%s", title[i]);

            int choix_type, nb_lig;
            do {    //Saisie du choix du type de la colonne
                printf("Choisissez le type de donnee de la colonne %d:\n"
                       "[1]: UNSIGNED INT\n"
                       "[2]: INT\n"
                       "[3]: CHAR\n"
                       "[4]: FLOAT\n"
            ...
            ENUM_TYPE type;     //Transformer le choix numéroté en ENUM_TYPE
            switch (choix_type) {   //Selon le numéro donné lors du choix, on initialise le type de la colonne
                case 1: {
                    type = UINT;
                    break;
                }
            ...
            do {
                ...
                scanf("%d", &nb_lig);
            } while (nb_lig < 0);   //Saisie sécurisée sur le nombre de valeurs à saisir dans la colonne créé

            for (int j=0; j<nb_lig; j++) {  //Boucle où on saisit les valeurs
                void *value;
    ...
    do {
        do {    //Tant que l'utilisateur n'a pas choisi une fonction
            ...
            rep = scanf("%d", &choix);
        } while (rep==0 || choix>16 || choix<0);  //Si le numero saisi est incorrecte ou que ce n'est pas un entier on recommence
        ...
        switch (choix){ //Selon le choix de la fonction, on utilise les fonctions suivantes
            ...
            case 2:{    //Afficher une partie des lignes du CDataframe selon une limite fournie par l’utilisateur
                int tete, queue;
                printf("Entrez l'intervalle de ligne a afficher\n Debut/Fin :");    //L'utilisateur entre les intervalles à afficher
                scanf("%d/%d", &tete, &queue);
            ...
            case 3: {   //Afficher une partie des colonnes du CDataframe selon une limite fournie par l’utilisateur
                ...
                do {    //Saisie sécurisée
                    ...
                    scanf("%d/%d", &tete, &queue);
                } while (tete < 0 || queue >= nb_col || tete > queue);
            ...
            case 4:{    //Ajouter une ligne de valeurs au CDataframe
                ...
                void* value;
                afficher_nom_col(cdf, nb_col);
                printf("Entrez l'indice de la colonne ou ajouter la valeur :"); //Saisie de la colonne où ajouter la valeur
                scanf("%d", &indice);
                printf("Entrez la valeur a ajouter:"); //Saisie de la valeur à ajouter
            ...
            case 5:{    //Supprimer une ligne de valeurs du CDataframe
                int indice;
                afficher_nom_col(cdf, nb_col);  //On affiche les noms des colonnes et leurs indices
            ...
            case 9:{    // Vérifier l’existence d’une valeur (recherche) dans le CDataframe
                ...
                if(existence == 1){   //On affiche son existence
                    printf("La valeur \"%s\" existe dans le CdataFrame\n", val);
                } else{
            ...
            case 12:{   // Afficher le nombre de lignes
                for(int i=0; i<nb_col; i++){    // On affiche le nom et le nombre de lignes de chaque colonne
                    printf("%s : %d ligne(s)\n",cdf[i]->title, cdf[i]->size);
                }
                break;
            }
            case 13: {  //Afficher le nombre de colonnes
                printf("Le CDataFrame contient %d colonne(s)\n", nb_col);   //On affiche le nombre de colonnes du CDataFrame
                break;
            }
            ...
            case 15:{   //Nombre de cellules contenant une valeur supérieure à x (x donné en paramètre)
                ...
                printf("Il y a %d cellule(s) qui contiennent une valeur superieure a celle donnee\n", nb_cellule_val_sup(cdf, nb_col, type, value));
                break;
            }
            case 16:{   //Nombre de cellules contenant une valeur inférieure à x(x donné en paramètre)
                ...
                printf("Il y a %d cellule(s) qui contiennent une valeur inferieure a celle donnee\n", nb_cellule_val_inf(cdf, nb_col, type, value));
                break;
            }
        }
        printf("Voulez-vous contiuer le programme ?\n"
               "[0]: NON\n"
               "[1]: OUI\n");
        scanf("%d", &choix);
